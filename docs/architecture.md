...
答辩时还可以主动补一句限定，避免把算法说得过满：

> 需要说明的是，当前连接管理、离线判定和发送时机控制采用的是 sleep-aware 的启发式近似策略，它利用 `sleep/work/next_wake` 与路径睡眠预算来减少误判和窗口错过，目标是工程上足够稳健，而不是宣称严格的全局最优。
...
> 当前 Stockman 采用 `Qt Widgets + C++20`，是为了优先保证控制逻辑、页面状态和交互稳定；如果后续继续产品化并追求更现代的表现层，再考虑向 `QML/Kirigami` 演进。也就是说，当前实现关注“先稳”，后续路线关注“更美观、更现代”。
