...
- **Stockman（GUI 客户端）**：桌面客户端，通过 gRPC 与 Kelpie 交互（见 `clientui/`）。`academic` 分支已将其从旧版 Qt6 C++ 客户端重写为基于 Wails v3 + Vue 3 + Vite 的轻量演示客户端，聚焦连接管理、拓扑总览、节点详情、事件时间线、演示控制台五类视图，适合答辩现场展示。
