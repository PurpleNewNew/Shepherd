...
本文档描述在 `academic` 分支上基于 **Wails v3.0.0-alpha.85 + Vue 3 + Vite** 的 Stockman 桌面客户端的构建与开发环境。旧版 Qt6 C++ Stockman 已在 `academic` 分支整体重写替换；历史清单仅在 `docs/stockmanchecklist.md` 里保留作为回归参考，并不适用于当前代码。

## 运行环境

macOS（Apple Silicon 已验证）、Linux、Windows 原生均可构建；开发时只需以下三件：

- **Go**：>= 1.25（Wails v3 alpha.85 的最低要求；仓库整体已在 Go 1.26 上验证）
...
可选：Wails v3 CLI 仅用于 `wails3 dev` 热重载和 bindings 生成，`make stockman` 的非热重载构建不需要它。
...
如果修改了 `clientui/backend/service` 暴露给前端的 facade 方法，先运行：
...
但直接浏览器访问时，Wails v3 的桌面运行时环境和后端 bindings 不存在，所有后端调用都会失败——这是预期的，浏览器只适合调 CSS/布局。实际功能必须通过 Wails 窗口测试。
...
- **Wails v3.0.0-alpha.85**：多窗口 + 前端静态资源 embed + Go ↔ JS bindings + 应用事件系统
