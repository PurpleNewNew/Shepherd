...
## 提交与 Pull Request 规范
提交信息统一使用中文前缀，格式为 `[类型] 简要说明`，其中类型限定为 `[功能]`、`[修复]`、`[优化]`、`[重构]`、`[其他]`。例如：`[功能] 增加 DTN 回放统计导出`、`[修复] 修正节点重连后的会话恢复`、`[优化] 降低 gossip 同步日志噪音`。Pull Request 应说明行为变化、列出已执行命令（如 `make test`、`make check`、`make regress`），关联相关 issue 或实验记录；若修改 `clientui/` 界面或交互，应附带截图。
